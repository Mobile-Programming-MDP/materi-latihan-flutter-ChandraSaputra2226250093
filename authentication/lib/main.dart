//import paket-paket yang dubutuhkan
import 'package:flutter/material.dart';
import 'package:encrypt/encrypt.dart' as encrypt;
import 'package:logger/logger.dart';
import 'package:shared_preferences/shared_preferences.dart';

//fungsi main yang menjalankan aolikasi
void main() {
  runApp(const MyApp());
}
...
class SignUpScreen extends StatelessWidget {
  ...
  void _performSignUp(BuildContext context){
    try {
      ...
      //Memberikan apakah username atau password kosong sebelum melanjutkan sign-up
      if (username.isNotEmpty && password.isNotEmpty) {
        final encrypt.Key key = encrypt.Key.fromLength(32);
        final iv = encrypt.IV.fromLength(16);
        
        final encrypter = encrypt.Encrypter(encrypt.AES(key));
        final encrypedUsername = encrypter.encrypt(username, iv: iv);
        final encrypedPassword = encrypter.encrypt(password, iv: iv);

        _saveEncryptedDataToPrefs(
          prefs,
          encrypedUsername.base64,
          encrypedPassword.base64,
          key.base64,
          iv.base64,
        ).then((_) {
  ...
  Future<void> _saveEncryptedDataToPrefs(
    ...
    await sharedPreferences.setString('username', encrypedUsername);
    await sharedPreferences.setString('password', encrypedPassword);
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: <Widget> [
            ...
            ElevatedButton(
              onPressed: (){
                _performSignIn(context);  //Tombol sign in
              }, 
        ...
        _retrieveAndDecryptDataFromPrefs(prefs).then((data) {
          if (data.isNotEmpty) {
            ...
            if (username == decryptedUsername && password == decryptedPassword) {
              ...
              _logger.e('Userename or password is incorrection');
            }
          } else{
